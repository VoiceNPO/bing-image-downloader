...
export const setFetchAPI = (_fetch: typeof fetch) => (config.fetch = _fetch);
export const setLogLevel = (level: number) => (config.logLevel = level);
export const setCookie = (cookie: string) => (config.cookie = cookie);

function validateFetchAPI() {
  if (!config.fetch) {
    throw new Error(
      `'fetch' is not defined.  Please either polyfill or set it before attempting to generate images with the ${name} package`
    );
  }
}

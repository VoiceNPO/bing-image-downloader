...
export async function fetchImageURLs(prompt: string): Promise<string[]> {
...
type encodedPrompt = { encodedPrompt: string };
type requestID = { requestID: string };
type queryURL = { queryURL: string };
type queryResponse = { queryResponse: Response };
type imagePageURL = { imagePageURL: string };
type imagePageHTML = { imagePageHTML: string };
type generatedImageURLs = { generatedImageURLs: string[] };

const getQueryURL = ({ encodedPrompt }: encodedPrompt): queryURL => ({
  queryURL: `${BASE_URL}/?q=${encodedPrompt}&rt=3&FORM=GENCRE`,
});

const getImagePageURL = ({
  requestID,
  encodedPrompt,
}: requestID & encodedPrompt): imagePageURL => ({
  imagePageURL: `${BASE_URL}/async/results/${requestID}?q=${encodedPrompt}`,
});

const encodePrompt = ({ prompt }: prompt): encodedPrompt => ({
  encodedPrompt: escape(prompt),
});

function validatePageHTML(html: string) {
  if (html.indexOf(BLOCKED_TEXT) > 0) {
    throw new Error(BLOCKED_RESPONSE);
  }
}

async function fetchQueryPage({ prompt, queryURL }: prompt & queryURL): Promise<queryResponse> {
...
const extractRequestID = ({ queryResponse }: queryResponse): requestID => {
...
async function awaitImageGen({ imagePageURL }: imagePageURL): Promise<imagePageHTML> {
...
function parseResponsePage({ imagePageHTML }: imagePageHTML): generatedImageURLs {
  validatePageHTML(imagePageHTML);

  const matchedCreateURLs = imagePageHTML.matchAll(
    /<div class="imgpt"><a\b[^>]+?href="\/images\/create\/.*?<img\b[^>]+?src="(http.*?)"/gm
  );
